...
def index(request):
    if request.user.role == "student":
        ...
        student = Student.objects.select_related("school", "class_name", "section").get(
            user=request.user
        )
...
User = (
    get_user_model()
)  # Django ka custom/default user model fetch karne ka sahi tarika
...
def create_student(request):
    ...
    if request.method == "POST":
        form = StudentForm(
            request.POST, request.FILES, school=school_obj, teacher=teacher
        )
        ...
        if form.is_valid():
            ...
            base_username = (
                admission_no if admission_no else f"{stu_fir_name}_{roll_no}".lower()
            )
            username = base_username.replace(" ", "_").lower()
            temp_password = (
                f"{stu_fir_name.lower()}@123" if stu_fir_name else "scho1234"
            )
            ...
            messages.success(
                request,
                f"Student '{stu_fir_name} {stu_las_name}' enrolled successfully!",
            )
